...
export function Logo({
  ...
  return (
    <Link href="/" className="flex items-center gap-3" aria-label={`${name} home`}>
      <span
        className={`relative flex h-12 w-12 items-center justify-center overflow-hidden rounded-lg shadow-lg shadow-brand-900/20 ${
      ...
      <span className="leading-tight">
        <span className={`block text-base font-black ${inverted ? "text-white" : "text-ink"}`}>
          {name}
        </span>
        <span
          className={`block text-xs font-semibold uppercase ${
            inverted ? "text-blue-100" : "text-brand-600"
          }`}
        >
          {tagline}
        </span>
