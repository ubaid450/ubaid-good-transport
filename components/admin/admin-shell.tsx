    ...
    <div className="min-h-screen bg-slate-100">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 overflow-y-auto border-r border-slate-200 bg-brand-900 p-4 text-white lg:block">
        <div className="rounded-lg bg-white/10 p-4">
          ...
          <p className="mt-1 text-xs font-semibold uppercase text-blue-100">
            Admin Control Panel
          </p>
        </div>
        ...
        <form action="/api/admin/logout" method="POST">
          <button
            type="submit"
            className="mt-6 flex w-full items-center gap-3 rounded-md border border-white/20 px-3 py-3 text-sm font-bold text-white transition hover:bg-white hover:text-brand-900"
          >
        ...
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
          <div className="container-pad flex min-h-16 items-center justify-between gap-4 py-3">
            ...
            <form action="/api/admin/logout" method="POST" className="lg:hidden">
              <button
                type="submit"
                className="focus-ring rounded-md border border-slate-200 px-4 py-2 text-sm font-bold text-ink"
              >
                Sign out
              </button>
            </form>
          ...
          <nav className="container-pad flex gap-2 overflow-x-auto pb-3 lg:hidden">
            {adminLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="shrink-0 rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-ink"
              >
                {link.label}
              </Link>
            ))}
